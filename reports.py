...
# ----------------- AUTOMATED DAILY PIPELINE LOGIC -----------------
@st.cache_resource
def start_auto_fetcher_thread():
    import threading
    import time
    def loop():
        while True:
            try:
                run_auto_check_logic(is_automated=True)
            except Exception as e:
                print(f"Error in auto fetch background loop: {e}")
...
try:
    start_auto_fetcher_thread()
except Exception as e:
    pass

st.sidebar.markdown("---")
st.sidebar.markdown("### ⏰ Automated Daily Fetch")
with st.sidebar.expander("Auto-Fetch Status & Config", expanded=False):
    import json
    import os
    ...
    if os.path.exists(status_path):
        try:
            with open(status_path, "r") as sf:
                status_data.update(json.load(sf))
        except:
            pass
            
    is_enabled = st.checkbox("Enable Automated Daily Check", value=status_data.get("enabled", True))
    if is_enabled != status_data.get("enabled", True):
        status_data["enabled"] = is_enabled
        try:
            with open(status_path, "w") as sf:
    ...
    sched_hour = st.number_input("Scheduled Hour (24h format):", min_value=0, max_value=23, value=status_data.get("scheduled_hour", 9))
    if sched_hour != status_data.get("scheduled_hour", 9):
        status_data["scheduled_hour"] = sched_hour
        try:
            with open(status_path, "w") as sf:
                json.dump(status_data, sf, indent=2)
        except:
    ...
    st.write(f"**Last Run:** `{status_data.get('last_run_time', 'Never')}`")
    st.write(f"**Status:** `{status_data.get('last_run_status', 'Idle')}`")
    
    if st.button("🔄 Run Auto-Fetch Now", type="secondary", use_container_width=True):
        with st.spinner("Running manual check and merge..."):
            run_auto_check_logic(is_automated=False)
            st.toast("Auto-fetch process completed!")
            st.rerun()
            
    st.markdown("**Recent Logs:**")
    logs_box = "\n".join(status_data.get("recent_logs", ["No logs yet."])[-10:])
    st.text_area("Worker Logs", value=logs_box, height=150, disabled=True)
